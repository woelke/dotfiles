...
def test_flag_parser():
    assert Flag(Exact("a"), key="key").parse("a".split(" "))["key"] == True
    assert Flag(Exact("a"), key="key").parse("b".split(" "))["key"] == False
    assert Flag(Exact("a") + Exact("b"), key="key").parse("a b".split(" "))["key"] == True
    with pytest.raises(ParseError):
